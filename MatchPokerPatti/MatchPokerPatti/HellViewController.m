...
-(void)loaggames{
    self.muarrymuag = [[NSMutableArray alloc]init];
           ...
           // 生成1到8的随机排列
           NSMutableArray *randomNumbers = [NSMutableArray arrayWithCapacity:24];
           for (int i = 1; i <= 24; i++) {
...
- (void)setupGridView {
    _lastint=@"";
    CGFloat gridSize = (self.backview.bounds.size.width-50)/8;
    CGFloat padding = (self.backview.bounds.size.height-50)/6;
    for (int i = 0; i < 6; i++) {
        for (int j = 0; j < 8; j++) {
            ...
            [button setImage:[UIImage imageNamed:@"backimage2"] forState:UIControlStateNormal];
